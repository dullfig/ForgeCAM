...
pub fn fillet_edges_into(
    ...
    for &face_idx in &face_list {
        ...
        build_face_from_vert_idxs(topo, geom, &vert_idxs, new_surface_id, new_shell_idx, &mut he_map);
    }

    // Add fillet faces.
    for (poly, surface) in fillet_faces {
...
mod tests {
    ...
    fn test_fillet_one_edge_euler() {
        ...
        let filleted = fillet_edges_into(&mut topo, &mut geom, solid, &[edge], 0.3, 4).unwrap();
    ...
    fn test_fillet_tessellation() {
        ...
        let filleted = fillet_edges_into(&mut topo, &mut geom, solid, &[edge], 0.3, 4).unwrap();
        let shell = topo.solids.get(filleted).shell;
    ...
    fn test_fillet_two_opposite_edges() {
        ...
        let filleted = fillet_edges_into(&mut topo, &mut geom, solid, &[e1, e2], 0.3, 4).unwrap();
        verify_euler(&topo, filleted);
        verify_all_twins(&topo, filleted);
