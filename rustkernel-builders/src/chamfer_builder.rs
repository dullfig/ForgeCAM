...
pub fn chamfer_edges_into(
    ...
    // Add chamfer faces.
    for (poly, surface) in chamfer_faces {
...
mod tests {
    ...
    fn test_chamfer_one_edge_euler() {
        ...
        let chamfered = chamfer_edges_into(&mut topo, &mut geom, solid, &[edge], 0.3).unwrap();
    ...
    fn test_chamfer_two_opposite_edges() {
        ...
        let chamfered = chamfer_edges_into(&mut topo, &mut geom, solid, &[e1, e2], 0.3).unwrap();
        verify_euler(&topo, chamfered);
        verify_all_twins(&topo, chamfered);
