...
pub fn validate_solid(topo: &TopoStore, solid: SolidIdx) -> DiagnosticReport {
    ...
    let solid_data = topo.solids.get(solid);
    let genus = solid_data.genus;

    // Validate each shell independently; collect totals across all shells
    let mut all_faces: Vec<FaceIdx> = Vec::new();
    for &shell_idx in &solid_data.shells {
        all_faces.extend_from_slice(&topo.shells.get(shell_idx).faces);
    }
    let faces = &all_faces;
...
mod tests {
    ...
    fn make_tetrahedron(topo: &mut TopoStore) -> SolidIdx {
        ...
        let solid_idx = topo.solids.alloc(Solid {
            shells: vec![Idx::from_raw(0)], // placeholder
            genus: 0,
        });
