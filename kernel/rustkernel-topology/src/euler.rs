...
// ─── KEV same-loop variant ──────────────────────────────────────────────────

/// Remove edge E and merge V_kill into V_keep, with support for the case
/// where both half-edges are in the same loop (spike topology: `...P → V → P...`).
///
/// Standard KEV uses `find_prev_he` with separate loops for each HE. When both
/// HEs are in the same loop, the second `find_prev_he` may find a dead HE as
/// predecessor. This variant detects that case and chains through correctly.
///
/// Also works correctly for the normal cross-loop case.
///
/// Euler: V-1, E-1, F unchanged.
pub fn kev_same_loop(
    topo: &mut TopoStore,
    edge: EdgeIdx,
    v_keep: VertexIdx,
) -> Result<KevResult, EulerError> {
    let _span = debug_span!("kev_same_loop", edge = edge.raw(), v_keep = v_keep.raw()).entered();
    ...
    let loop_keep = topo.half_edges.get(he_from_keep).loop_ref;
    let loop_kill = topo.half_edges.get(he_from_kill).loop_ref;
    let next_from_keep = topo.half_edges.get(he_from_keep).next;
    let next_from_kill = topo.half_edges.get(he_from_kill).next;
    let same_loop = loop_keep == loop_kill;

    if same_loop {
        // Both HEs in the same loop — find predecessors BEFORE any mutations.
        let prev_keep = find_prev_he(topo, loop_keep, he_from_keep);
        let prev_kill = find_prev_he(topo, loop_keep, he_from_kill);

        // Bypass he_from_keep
        topo.half_edges.get_mut(prev_keep).next = next_from_keep;

        // Bypass he_from_kill — but prev_kill might be he_from_keep (already dead).
        if prev_kill == he_from_keep {
            // Spike case: ...prev_keep → he_from_keep → he_from_kill → next_from_kill...
            // After first bypass: prev_keep.next = next_from_keep = he_from_kill.
            // Now bypass he_from_kill: prev_keep.next = next_from_kill.
            topo.half_edges.get_mut(prev_keep).next = next_from_kill;
        } else if prev_kill == he_from_kill {
            // Should not happen in well-formed topology
            topo.half_edges.get_mut(prev_keep).next = next_from_kill;
        } else {
            topo.half_edges.get_mut(prev_kill).next = next_from_kill;
        }
        ...
        // Fix loop entry
        let entry = topo.loops.get(loop_keep).half_edge;
        if entry == he_from_keep || entry == he_from_kill {
            topo.loops.get_mut(loop_keep).half_edge = next_from_kill;
        }
    } else {
        // Normal cross-loop case: same as standard KEV
        let prev_keep = find_prev_he(topo, loop_keep, he_from_keep);
        topo.half_edges.get_mut(prev_keep).next = next_from_keep;

        let prev_kill = find_prev_he(topo, loop_kill, he_from_kill);
        topo.half_edges.get_mut(prev_kill).next = next_from_kill;

        if topo.loops.get(loop_keep).half_edge == he_from_keep {
            topo.loops.get_mut(loop_keep).half_edge = next_from_keep;
        }
        if topo.loops.get(loop_kill).half_edge == he_from_kill {
            topo.loops.get_mut(loop_kill).half_edge = next_from_kill;
        }
    }

    // Rewrite all HE origins from v_kill -> v_keep across the shell
    let face_of_loop = topo.loops.get(loop_keep).face;
    let shell_idx = topo.faces.get(face_of_loop).shell;
    let faces: Vec<FaceIdx> = topo.shells.get(shell_idx).faces.clone();
...
mod tests {
    ...
    #[test]
    fn test_kev_same_loop_normal_case() {
        // kev_same_loop works like regular KEV when HEs are in different loops.
        let mut topo = TopoStore::new();
        let solid = make_test_tetrahedron(&mut topo);
        assert_eq!(count_vef(&topo, solid), (4, 6, 4));

        let h = find_he(&topo, solid, 0, 1);
        let s = semv(&mut topo, h, 95, 0, 0);
        assert_eq!(count_vef(&topo, solid), (5, 7, 4));

        let [ke0, ke1] = topo.edges.get(s.new_edge).half_edges;
        let ko0 = topo.half_edges.get(ke0).origin;
        let ko1 = topo.half_edges.get(ke1).origin;
        let v_keep = if ko0 == s.new_vertex { ko1 } else { ko0 };

        kev_same_loop(&mut topo, s.new_edge, v_keep).expect("kev_same_loop normal case");
        assert_eq!(count_vef(&topo, solid), (4, 6, 4));

        let report = validate_solid(&topo, solid);
        assert!(report.is_valid(), "After kev_same_loop: {:?}", report.errors());
    }

    #[test]
    fn test_kev_same_loop_roundtrip() {
        // SEMV + kev_same_loop roundtrip preserves topology.
        let mut topo = TopoStore::new();
        let solid = make_test_tetrahedron(&mut topo);

        // Split two different edges, then undo both with kev_same_loop
        let h01 = find_he(&topo, solid, 0, 1);
        let s1 = semv(&mut topo, h01, 70, 0, 0);
        let h23 = find_he(&topo, solid, 2, 3);
        let s2 = semv(&mut topo, h23, 71, 0, 0);
        assert_eq!(count_vef(&topo, solid), (6, 8, 4));

        // Undo both
        let [e1a, e1b] = topo.edges.get(s1.new_edge).half_edges;
        let v1_keep = if topo.half_edges.get(e1a).origin == s1.new_vertex {
            topo.half_edges.get(e1b).origin
        } else {
            topo.half_edges.get(e1a).origin
        };
        kev_same_loop(&mut topo, s1.new_edge, v1_keep).expect("undo s1");

        let [e2a, e2b] = topo.edges.get(s2.new_edge).half_edges;
        let v2_keep = if topo.half_edges.get(e2a).origin == s2.new_vertex {
            topo.half_edges.get(e2b).origin
        } else {
            topo.half_edges.get(e2a).origin
        };
        kev_same_loop(&mut topo, s2.new_edge, v2_keep).expect("undo s2");

        assert_eq!(count_vef(&topo, solid), (4, 6, 4));
        let report = validate_solid(&topo, solid);
        assert!(report.is_valid(), "After double roundtrip: {:?}", report.errors());
    }
}
