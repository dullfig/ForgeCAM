...
/// Trim an intersection line to a single face boundary.
pub fn trim_line_to_face(
    line: &IntersectionLine,
    topo: &TopoStore,
    geom: &dyn GeomAccess,
    face: FaceIdx,
) -> Vec<TrimmedSegment> {
    let intervals = clip_line_to_face(line, topo, geom, face);
    intervals
        .iter()
        .filter(|(s, e)| e - s > 1e-10)
        .map(|&(start, end)| TrimmedSegment {
            t_start: start,
            t_end: end,
            start_point: line.origin + start * line.direction,
            end_point: line.origin + end * line.direction,
...
/// Trim a sampled curve to a single face boundary.
///
/// Like `trim_sampled_curve` but clips each chord against only one face.
pub fn trim_sampled_curve_single_face(
    points: &[Point3],
    topo: &TopoStore,
    geom: &dyn GeomAccess,
    face: FaceIdx,
    ...
    for i in 0..points.len().saturating_sub(1) {
        ...
        let intervals = clip_line_to_face(&chord_line, topo, geom, face);

        for &(a_start, a_end) in &intervals {
            let start = a_start.max(0.0);
            let end = a_end.min(1.0);
            if end - start > 1e-10 {
                result.push(TrimmedSegment {
                    t_start: start,
                    t_end: end,
...
/// Trim an intersection circle to a single face boundary.
///
/// Returns segments representing the portion of the circle within the face.
pub fn trim_circle_to_face(
    circle: &IntersectionCircle,
    topo: &TopoStore,
    geom: &dyn GeomAccess,
    face: FaceIdx,
    ...
    let points = sample_circle(circle, DEFAULT_CHORD_COUNT);
    trim_sampled_curve_single_face(&points, topo, geom, face)
}

/// Trim an intersection ellipse to a single face boundary.
pub fn trim_ellipse_to_face(
...
mod tests {
    ...
    fn test_clip_line_to_cylinder_face() {
        ...
        let shell = topo.solids.get(cyl).outer_shell();
    ...
    fn test_clip_line_to_cone_face() {
        ...
        let shell = topo.solids.get(cone).outer_shell();
        let lateral_face = topo.shells.get(shell).faces.iter().find(|&&f| {
            let sid = topo.faces.get(f).surface_id;
    ...
    fn test_trim_circle_box_sphere() {
        ...
        let shell_box = topo.solids.get(bx).outer_shell();
        ...
        let shell_sp = topo.solids.get(sp).outer_shell();
