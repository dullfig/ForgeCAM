...
/// Result of building a boolean result solid, including face copy mapping.
pub struct BuildResult {
    pub solid: SolidIdx,
    /// Map from source face (in the pre-boolean solid) → new face in the result.
    pub face_copy_map: Vec<(FaceIdx, FaceIdx)>,
}
...
///
/// Returns both the result solid and a mapping from source → result face indices,
/// used for shape evolution tracking.
pub fn build_result_solid(
    ...
    topo.solids.get_mut(result_solid).shells = vec![result_shell];

    // Track source → result face mapping.
    let mut face_copy_map: Vec<(FaceIdx, FaceIdx)> = Vec::new();
    ...
    for &face_idx in &selected.keep_from_a {
        ...
        face_copy_map.push((face_idx, new_face));
    ...
    // Merge coincident vertices (tolerance matches face_splitter snap tolerance).
    merge_coincident_vertices(topo, geom, &all_new_faces, 1e-6);

    // Match twins.
    match_twins(topo, geom, &all_new_faces);

    // Validate.
    validate(topo, &all_new_faces, result_solid)?;
...
///
/// Accepts V - E + F = 2 - 2*genus. The genus is auto-detected from the
/// topology: if V - E + F is even and <= 2, genus = (2 - (V-E+F)) / 2.
fn validate(topo: &mut TopoStore, faces: &[FaceIdx], result_solid: SolidIdx) -> Result<(), BuildError> {
    ...
    // V - E + F = 2 - 2*genus. Genus must be non-negative integer.
    // euler must be even, and <= 2.
    if euler > 2 || euler % 2 != 0 {
        return Err(BuildError::EulerViolation { v, e, f });
    }

    let genus = ((2 - euler) / 2) as u32;
    topo.solids.get_mut(result_solid).genus = genus;
...
mod tests {
    ...
    fn test_sew_cube_from_standalone_faces() {
        ...
        assert!(!build.face_copy_map.is_empty(), "Should have face copy map");
    }
}
